              ...
              <v-list>
                <v-list-item>
                  <v-list-item-action>
                    <vue-json-to-csv
                      :json-data="dataDownload"
                      :csv-title="'Cycle Time Variance Report'"
                    >
...
export default {
  ...
  data() {
    return {
                                        crumbs: ['Home', 'Report Menu','Cycle Time Variance Report'],
      items: [
        {
          text: "Home",
          disabled: false,
        ...
        {
          text: "Cycle Time Variance Report",
          disabled: true,
        },
      ],
      showWarning: false,
  ...
  methods: {
    ...
    createPDF() {
      ...
      doc.text(14, 7.5, "Cycle Time Variance Report");
      doc.addImage(logo, "png", 247, 3, 35, 5.5);
      let columnHeaders = [
        "Machine Name",
        "Time Stamp",
        "Current Cycle Time(seconds)",
        "Previous Cycle Time(seconds)",
        "CT Variance %"
      ];
      let rows = [];
      content.items.forEach((data, index) => {
        if (
          ...
          let tempRow = [
            data.machine_name || "",
            data.timestamp || "",
            data.current_cycle_time || "",
            data.previous_cycle_time || "",
            data.ct_variance || "",
          ];
          rows.push(tempRow);
        }
      ...
      let d = new Date();
      doc.save(
        `Cycle_Time_Variance_Report ${d.toISOString().slice(0, 10) +
          " " +
          d
            .toTimeString()
            .split("(")[0]
    ...
    searchNew: function(machine_id, startDate, endDate) {
      ...
      let config = {
        method: "get",
        url: `${constants.backend_url}/getCycleTimeVarianceReportData${params}`,
        headers: {
