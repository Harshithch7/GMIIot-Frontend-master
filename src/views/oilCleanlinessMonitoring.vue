        ...
        <v-card id='ins1' class="oee-card oee-card-white card-shadow " height="280px" width="350px"
         ...
         style="overflow-y: auto; " >
          <v-card-text  >
           ...
           <div v-bind:key="epak.id" v-for="epak in alertsData" >
<v-row>
<v-col v-if="showInsights">
                       <v-container>

                       <v-card id='ins1'  class="oee-card oee-card-white card-shadow " >
                               <v-row align="center"  >
                              <v-icon color="primary" class="pt-3 pl-3 pb-3">icon-insights</v-icon>
              <v-card-title class=" pt-3 pl-3 pb-3" > Violation Trends</v-card-title>
            </v-row>
                ...
                <apexchart
                  ...
                  :series="epak.series"
                   ></apexchart>
                       ...
                       <v-card id='ins2'  class="oee-card oee-card-white card-shadow " >
                               <v-row align="center"  >
                              <v-icon color="primary" class="pt-3 pl-3 pb-3">icon-insights</v-icon>
              <v-card-title class=" pt-3 pl-3 pb-3" > Violation Trends</v-card-title>
            </v-row>
...
export default {
  ...
  data: () => ({
    ...
    showresult: false,
    showLoader: false,
                  crumbs: ['Home', 'Oil Cleanliness Monitoring'],
                  alertsData:[],
oil_name: "",
    insightsData: [],
          apqData: [],
  ...
  methods: {
      ...
      if (crumb === 'Home')
      {
        if (`/${sessionStorage.getItem("route_first_option")}` == '/oilCleanlinessMonitoring')
        {
    ...
    Changeiconaspertitle(value) {
      ...
      else if (value == "NAS") return "icon-bolt";
    },
 timeConverter: (UNIX_timestamp) => {
      ...
      let config = {
          method: 'get',
          url: `${constants.backend_url}/getIPDMonitoringData${params}`,
          headers: { 
            'Authorization': `Bearer ${sessionStorage.getItem('token')}`
          }
        };
      ...
      axios(config)
        .then((response) => {
          ...
          try {
            let condemn_list =
              response.data.data[0].condemning_limit_list || [];
                          let alerts_stack_data = response.data.data[0].alerts_data || [];
            ...
            // console.log("Checking alerts stack data",alerts_stack_data);
            let dates = alerts_stack_data[1] || [];
            let stack_dic = alerts_stack_data[0] || [];
            let params_list = alerts_stack_data[2] || [];
            console.log("checking dates and stack data",dates,stack_dic);
            let parameter_value_all_month = [];
            let total_params = [];
            // this.alertsData = [];
            for (let k=0;k<params_list.length;k++)

            {
              console.log("Checking parameter ids",params_list[k],stack_dic,stack_dic.length);

              for (let j=0;j<stack_dic.length; j++)
              {
                          for (let i=0;i<dates.length; i++)
            {
              {
              if (dates[i] == stack_dic[j]['date'] && stack_dic[j]['parameter_id'  ] == params_list[k])
              {
                parameter_value_all_month.push(stack_dic[j]['count']);
              }
            ...
            {
              if (this.parameters[i]['parameter_id'] ===params_list[k])
              {
                param_name = this.parameters[i]['parameter_name'] 
                console.log("Checking after coniditon statisfied",param_name);
              }
            }
            console.log("Checking out of condition",param_name);
              let dicData = {
                name: param_name,
                data: parameter_value_all_month
              }
              total_params.push(dicData);
              parameter_value_all_month = [];
            }
            // console.log("checking result of all params",total_params,dates);
          ...
          apexchartOptions: {
            ...
            let add = [];
            add.push(chartData);
                                    // console.log("checking lsit aft",add);

          this.alertsData = add || [];
            // console.log("checking apexcharts",this.alertsData);
   ...
   insights:eligibleInsight
    ...
    intervalFetchData: function(machine_id, startDate, endDate) {
      ...
      this.end_date = endDate;
      console.log(
        "Autorefresh state inside interfetch data oilcleanlinessmonitoring",
        this.checkbox
      );
      this.searchNew(this.machine_id_Selected, this.start_date, this.end_date);
                                this.assignparam(this.machine_id_Selected);
