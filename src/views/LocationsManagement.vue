        ...
        >
          Menu as Popover
            ...
            <v-card>
              <v-card-title>
                <span v-if="!isEdit"
                  ><h5 class="text-h5">Add New Location</h5>
                </span>
                <span v-if="isEdit"
                  ><h5 class="text-h5">Edit Location Details</h5>
                </span>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field
                        label="Price per kWh(rupees) "
                        v-model="editeditem.kWh_price"
                        required
                      ></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="12" md="12">
                      <label class="radiolabel">Active Status</label>
...
export default {
  ...
  data: () => ({
    isEdit: false,
    dataDownload: [],
                  crumbs: ['Home', 'Admin Menu','Locations Management'],
  ...
  methods: {
      ...
      let doc = new jsPDF();
      doc.text(14, 7.5, "Location Configuration");
    ...
    changeStatus(item, enable_status) {
      ...
      let config = {
        method: "post",
        url: `${constants.backend_url}/updateLocationData`,
        headers: {
    ...
    setGridData: function() {
      ...
      let config = {
        method: "get",
        url: `${constants.backend_url}/getLocationdetails`,
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        },
      };
    ...
    save() {
      ...
      if (
        this.editeditem.location_name === null ||
        this.editeditem.location_name === ""
      ) {
      ...
      const req = {
        location_id: this.isEdit ? this.editeditem.location_id : undefined,
      ...
      let url = this.isEdit
        ? `${constants.backend_url}/updateLocationData`
        : `${constants.backend_url}/locationDataEntry`;
      // let url = `${constants.backend_url}/machineDataEntry`;
